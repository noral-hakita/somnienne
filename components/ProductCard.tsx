...
export default function ProductCard({ product }: { product: MockProduct }) {
  ...
  return (
    ...
    >
      {/* The Image Area (Using Gradients as elegant placeholders) */}
          ...
          <button
            onClick={() => {
              addItem({
                ...
                name: product.name,
                price: product.price,
                image: '' // We'll add real images when we connect Supabase
              })
            }}
            className="w-full bg-espresso hover:bg-bronze text-ivory border border-espresso hover:border-bronze px-4 py-3 text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-2 transition-all duration-300"
          >
            Add to Wardrobe
          </button>
        </div>
