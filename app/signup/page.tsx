...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.auth.signUp({
      email,
      password,
      options: { data: { full_name: name } }, // stored as metadata — see Socratic question
    })
    ...
    <div className="min-h-screen flex items-center justify-center px-6 pt-24 pb-16">
      <div className="w-full max-w-md">
        ...
        <h1 className="font-serif text-4xl md:text-5xl font-light text-espresso text-center mb-10">
          Create your <span className="italic text-bronze">account</span>
        </h1>

        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label htmlFor="name" className="block text-[10px] uppercase tracking-[0.25em] text-taupe mb-2">
              Full name
            </label>
            <input
              id="name"
        ...
        <p className="text-center text-sm text-taupe mt-8">
          ...
          <Link href="/login" className="text-espresso underline underline-offset-4 hover:text-bronze transition-colors">
            Sign in
          </Link>
        </p>
      </div>
